...
from .transformer3D import build_transformer3D
...
class IntermDETR(nn.Module):
    """ IntermDETR module that performs 3D object detection """
    def __init__(self, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if not two_stage:
            ...
            """This block of code can be used if we increase the scales in unimatch"""
        ...
        if with_box_refine:
            ...
            self.transformer.decoder.dim_embed = self.dim_embed_3d  
    ...
    def forward(self, features, calibs, img_sizes, targets):
        ...
        srcs = []
        masks = []
        # for l, feat in enumerate(features): 'this module should be changed if there is multi-scale convolution in unimatch'
        ...
        masks.append(mask)        
        pos.append(self.pos_embedding(features))
        ...
        hs, init_reference, inter_references, inter_references_dim, enc_outputs_class, enc_outputs_coord_unact = self.transformer(
        ...
        for lvl in range(hs.shape[0]):
            ...
            # depth average + sigma
            depth_ave = torch.cat([((1. / (depth_reg[:, :, 0: 1].sigmoid() + 1e-6) - 1.) + depth_geo.unsqueeze(-1) ) / 3,  # + depth_map
                                    depth_reg[:, :, 1: 2]], -1)
...
def build_intermdetr(cfg):
    # backbone
    # backbone = build_backbone(cfg)

    # detr
    depthaware_transformer = build_transformer3D(cfg)
