...
from helpers.save_helper import get_checkpoint_state
from helpers.save_helper import load_checkpoint
from helpers.save_helper import save_checkpoint
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, (inputs, calibs, targets, info) in enumerate(self.train_loader):
            ...
            if type(inputs)==tuple:
                ...
                with torch.no_grad():
                    pred_disp = self.unimodel(left, right)['flow_preds']

                inputs = self.unimodel.upsampler[1].register_forward_hook(get_activation('backbone'))
            ...
            outputs = self.model(inputs, calibs, img_sizes, targets)############''' ___<>___ '''#################
