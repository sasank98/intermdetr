...
# os.chdir("/home/autonomy/stereo_camera/uniDETR/datasets")



from datasets.utils import angle2class
...
from datasets.kitti.pd import PhotometricDistort
...
class KITTI_Dataset(data.Dataset):
    def __init__(self, split, cfg):
        ...
        # basic configuration
        self.root_dir = cfg.get('root_dir','/home/autonomy/stereo_camera/data')
        self.split = split
        self.intermediate_layer = cfg.get('intermediate_layer')
        self.num_classes = 3
        self.max_objs = 50
        ...
        self.image_left_dir = os.path.join(self.data_dir, 'image_2')
        self.image_right_dir = os.path.join(self.data_dir, 'image_3')
        self.calib_dir = os.path.join(self.data_dir, 'calib')
        self.label_dir = os.path.join(self.data_dir, 'label_2')

        if self.intermediate_layer != '':
            self.array_dir = os.path.join(self.data_dir, self.intermediate_layer)

        # data augmentation configuration  True if split in ['train', 'trainval'] else
    ...
    def get_left_image(self, idx):
        img_file = os.path.join(self.image_left_dir, '%06d.png' % idx)
    ...
    def get_array(self, idx):
        img_file = os.path.join(self.array_dir, '%06d.npy' % idx)
        assert os.path.exists(img_file)
        tensor = torch.from_numpy(np.load(img_file))
        return tensor
    ...
    def __getitem__(self, item):
        ...
        """"NOTE: Data augmentation is not changed for stereo cameras and it is currently not being used """
        ...
        # collect return data
        if self.intermediate_layer == "":
            inputs = self.get_left_image(index),self.get_right_image(index)
        else:
            inputs = self.get_array(index)
